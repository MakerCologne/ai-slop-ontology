...
python3 -c "import pytest, yaml; print('  pytest + pyyaml ok')" || {
  echo "  fehlt: pip install pytest pyyaml hatchling"; exit 2; }
python3 -c "import hatchling" 2>/dev/null && echo "  hatchling ok" \
  || echo "  WARNUNG: hatchling fehlt -> test_packaging ueberspringt die Build-Tests"

step "1 Suite"
python3 -m pytest -q; check $?

step "2 Datendateien"
python3 -c "import json; json.load(open('ontology.json')); print('  ontology.json ok')"; check $?
python3 -c "import yaml; yaml.safe_load(open('ai_slop_ontology.yaml')); print('  yaml ok')"; check $?

step "3 Gates"
for s in check_consistency check_ssot check_doc_signals check_methodology check_signal_dod check_model_dynamics check_signal_reliability; do
  printf '  --- %s\n' "$s"; python3 "scripts/$s.py"; check $?
done
printf '  --- fp_baseline --check\n'; python3 scripts/fp_baseline.py --check; check $?
printf '  --- run_control_set\n';    python3 eval/run_control_set.py | tail -3; check ${PIPESTATUS[0]}
printf '  --- self_check_docs\n';    python3 scripts/self_check_docs.py | tail -3; check ${PIPESTATUS[0]}
...
python3 eval/run_benchmark.py --min-precision 1.0 --min-recall 0.99 | tail -1; check ${PIPESTATUS[0]}
